...
function createAccessToken(uid) {
  return jwt.sign({ uid }, process.env.JWT_ACCESS_SECRET, { expiresIn: "15m" });
}
function createRefreshToken(uid) {
  return jwt.sign({ uid }, process.env.JWT_REFRESH_SECRET, { expiresIn: "30d" });
}
...
router.post("/login", async (req, res) => {
  try {
    ...
    const userQ = await db.query("SELECT * FROM users WHERE email=$1", [email.toLowerCase()]);
